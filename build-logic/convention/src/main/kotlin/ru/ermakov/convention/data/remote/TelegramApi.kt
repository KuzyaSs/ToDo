...
class TelegramApiImpl(private val httpClient: HttpClient) : TelegramApi {
    override suspend fun upload(file: File, token: String, chatId: String): HttpResponse {
        return httpClient.post("$BASE_URL/bot$token/sendDocument") {
            ...
            setBody(
                MultiPartFormDataContent(
                    formData {
                        append(
                            "document", file.readBytes(), Headers.build {
                                append(
    ...
    override suspend fun sendMessage(message: String, token: String, chatId: String): HttpResponse {
