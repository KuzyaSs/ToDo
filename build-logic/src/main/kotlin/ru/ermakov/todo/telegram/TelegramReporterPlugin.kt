...
class TelegramReporterPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        val extension = project.extensions.create("telegramReporter", TelegramExtension::class)
        val telegramApi = TelegramApiImpl(HttpClient(OkHttp))
        androidComponents.onVariants { variant ->
            val artifacts = variant.artifacts.get(SingleArtifact.APK)
            project.tasks.register(
                "reportTelegramApkFor${variant.name.uppercaseFirstChar()}",
